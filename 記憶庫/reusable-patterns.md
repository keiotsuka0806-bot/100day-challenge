...
## 原子的インポート＋破損退避（localStorageアプリのデータ復元）
**使い道**: JSON書き出し/復元機能を持つlocalStorage完結アプリ全部。不正JSONで既存データが半壊する事故を防ぐ。
**実装例**: `開発部/hikitsugi-shokunin/app.js` のインポート処理（QA 2026-07-10で全面書き換え）
**注意点**: ① **全検証→一時オブジェクト→一括適用**の3段にする（メタだけ先に上書きして途中で例外＝半壊、が最悪パターン）。② 置換前に `confirm` で既存データ消去を明示。③ 破損したlocalStorage rawは捨てずに `_broken` キーへ退避してから初期化（後で救出できる）。④ AI縮退モックには `reason`(no_key / ai_error)を持たせ、障害時に「AI未接続」と嘘の表示をしない（絶対ルール1準拠）。

## localStorage写真の3層防御（縮む・先回り警告・巻き戻し）
**使い道**: 写真をbase64でlocalStorageに貯めるPWA全般（典型利用で5MB制限は現実に踏む＝写真60件で約6MB）
**実装例**: `開発部/jiyukenkyu-lab/app.js` の保存処理（JPEG品質0.62圧縮／容量3.8MB超で先回り警告トースト／save()失敗時の巻き戻し）
**注意点**: 巻き戻しだけだと「入力が黙って消える」最悪体験になる。**一杯になる前に知らせる先回り警告**が本体で、圧縮と巻き戻しはその両脇。全保存関数で `if (!save()) return;` を徹底する。

## 役割の異なる質問チェーン（AIに書かせず深さを作る）
**使い道**: 教育・内省系アプリで「AIが代筆すると体験が死ぬ／一問だけだと浅い」の両方を避けたいとき。
**実装例**: `開発部/kansou-no-doko/`（読書感想文: 一場面→本人の一言→2〜3問）
**骨子**: ① AIの出力は「本人の発言内の完全一致引用」と「次の質問1つ」だけに限定（感想文・例文・答え・評価は作らない）。② 質問は同じ深掘りの繰り返しでなく**役割を変えて段階化**する（場面との接続→理由・別の見方→変化・残る問い）。③ 各質問は**前の回答の具体的な言葉を受け取って**作る＝深さは長文生成でなく「受け取りの具体性」から生まれる。④ 最低2問・3問目は本人が続けるか選ぶ（尋問化を防ぐ）。⑤ 完成画面に残すのは本人の回答だけ。
**注意点**: 「前の回答を使う」制約がないと質問が一般論や登場人物の内心推測に流れる。制約は明文でプロンプトに入れ、本番出力で逸脱を実測確認する。
