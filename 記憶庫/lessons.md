...
- **[git hygiene／持ち越し解決]** 6/15から鳴り続けていた git hygiene NG（スキルカタログ複製のコミット＋`.env*.example` 混入）を 6/22 にようやく対処（複製スキルカタログ4本と.env系を追跡解除＝fe0378a）。**追記で持ち越さず対処すれば鳴り止む**を実証。今後は「NGを記録した翌朝に対処まで完了」をルールにする。
