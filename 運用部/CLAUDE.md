...
> **デプロイ前ゲート（必須）**：本番デプロイの前に、次の3点を**この順で**すべて通すこと。1つでも未通過ならデプロイしない。コードを最も変える Genius Council を先に、最終レビュー（QA→セキュリティ）を後に置く。
> 1. **Genius Council（3ループ）** をローカルプレビューに対して実施（下記「Genius Council 必須ルール（デプロイ前ゲート）」）
> 2. **QAチェック**（`/code-review`）
> 3. **セキュリティチェック**（`security-check` スキル / `/security-review`）で検出事項を解消（デプロイ直前の最終ゲート。通過後はコードを変えない）
...
> Genius Council の3ループ改善は**デプロイ前ゲート**で完了している前提（下記）。デプロイ後はその反映確認と共有可能判定のみ。

1. デプロイ完了URLにアクセスして動作確認
2. ログイン機能が動くか確認
3. 主要機能を1通り操作する
4. Release Check を実行する
5. 必要なら Visual Smoke Check を実行する

## Genius Council 必須ルール（デプロイ前ゲート）

**本番デプロイの前に**、全プロジェクトでローカルプレビュー（`firebase serve` / `vercel dev` / ローカルhttpサーバー）に対して以下を実行する。**通過するまでデプロイしない。**

```bash
cd /Users/kei/dev/100day-challenge
# --url にはローカルプレビューURL（例 http://localhost:8000）を渡す
node 運用部/scripts/post-deploy-genius-cycle.mjs --project 開発部/[project-name] --url [local-preview-url]
```

> スクリプト名は歴史的経緯で `post-deploy-genius-cycle.mjs` のままだが、運用上は**デプロイ前ゲート**として使う。

このスクリプトは、プロジェクト内容から適切な天才プロ集団を自動選定し、3ループ改善チケットを `デザイン部/reports/` に保存する。
同時に、実作業に分解した共通タスクカードを `運用部/tasks/[project-name]/GC-L1.md`、`GC-L2.md`、`GC-L3.md` に保存する。これはLexWorld専用ではなく、全プロジェクト共通の作業カード置き場。各カードには `Open / Doing / Done / Skipped` の状態と、変更ファイル・確認コマンド・プレビューURL確認の証拠欄を持たせる。

運用部はレポートを読んで、実装可能な改善を3回反映する。**3ループを反映して Genius Council を通過させてから本番デプロイし、公開URLでHTTP 200と主要ファイルの反映を確認する。**
...
状態遷移は以下に固定する（Genius Council はデプロイ前ゲート）。

`開発中 -> [デプロイ前ゲート: Genius Council 3ループ → QA → セキュリティ] -> デプロイ済み -> デプロイ後改善中（疎通・調整） -> 共有可能`
...
- 本番URLがHTTP 200
- **デプロイ前ゲートを通過済み**（Genius Council → QA → セキュリティ の3点をこの順で）
- QA部の最低限チェックが通っている
- セキュリティチェックの検出事項が解消済み
...
1. その日の運用日報を作成・更新する
2. Obsidian の日報(**1ファイルだけ**)を更新する
3. その日に変化したプロジェクトがあれば `project-registry.json` と Obsidian のプロジェクトノートも更新する
4. **企画10案テーブルの確認・確定**: Obsidian日報に朝会の `## 企画10案（朝会）` 表があるか確認する。無ければ `企画部/specs/YYYY-MM-DD-*.md` から復元して追加し、判定列を**史実**に更新する（実際に作った案=`★採用(制作)`／作ってドロップ=`制作→Drop`／未制作=`落選`）。罫線ASCIIではなくMarkdown表で書く（[[idea-10-recording-rule]] / 朝会SKILL Step 2-5）

| ファイル | パス |
|---|---|
| Daily Control Sheet | `運用部/daily/YYYY-MM-DD.md` |
| Obsidian 日報 | `/Users/kei/Documents/Kei/日報/YYYY-MM-DD.md` |
| Obsidian プロジェクトノート | `/Users/kei/Documents/Kei/100 Day Challenge/プロジェクト/Day N [Project].md` |

**2026-06-11の統一ルール（重要）:**

- Obsidianの日報は Vault直下の `日報/YYYY-MM-DD.md` **ただ1つ**。旧 `100 Day Challenge/日報/` と `100 Day Challenge/100日チャレンジ日報/` への三重書きは**廃止**（旧ファイルは `Archive/旧日報/` に退避済み）
- 日報には**詳細を書かない**。プロジェクトの詳細はプロジェクトノートに書き、日報からは `[[Day N プロジェクト名]]` でリンクして1行サマリだけ載せる
- フォーマットは Vault の `テンプレート/日報テンプレート.md` に従う（既存セクションがあれば追記、なければテンプレートから作成）
- 表の整形（パディング）を勝手に変えない。既存ファイルに追記するときは既存の整形に合わせる
...
## 今日やったこと
- [箇条書き。100日チャレンジ以外の活動もここに]

## 100日チャレンジ
- [[Day N プロジェクト名]] — [状態と1行サマリ。詳細はプロジェクトノートへ]
...
| 項目 | 内容 |
| --- | --- |
| 本日の選定 | [ProjectName]（Day N） |
| 完成物 | [1行説明] |
| URL | https://... |
| 状態 | [開発中 / デプロイ済み / 共有可能 / 稼働中] |
| 主な技術 | [スタック] |
...
タイトル直下のサマリー表は**必須**。状態やURLが変わったら表も更新する。
