...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.status(response.status).json({ error: formatOpenAiError(err, response.status) });
      return;
    }
    ...
    res.status(200).json(normalizeResult(JSON.parse(match[0])));
  } catch (e) {
    res.status(500).json({ error: e.message || '採点に失敗しました' });
  }
}

function formatOpenAiError(err, status) {
  const code = err?.error?.code || '';
  const type = err?.error?.type || '';
  const message = err?.error?.message || '';
  const raw = `${code} ${type} ${message}`.toLowerCase();

  if (raw.includes('insufficient_quota') || raw.includes('quota') || raw.includes('billing')) {
    return 'AI採点の利用枠が上限に達しています。管理者がOpenAIの課金設定または利用上限を確認するまで、しばらく採点できません。';
  }
  ...
  return 'AI採点サーバーでエラーが発生しました。時間をおいてもう一度試してください。';
}
