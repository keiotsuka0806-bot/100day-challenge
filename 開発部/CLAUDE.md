...
| RideDex | `ride-dex/` | デプロイ済み（Day27・3ゲート通過・本番疎通OK／https://ridedex-kei.vercel.app）。街の車・バイクをカメラ→OpenAI Vision鑑定（メーカー/車種/世代+年式レンジ/確度%/レア度/豆知識）→図鑑コレクション。"宝探し"UX。年式は単年でなく世代レンジ／詳しい人が訂正可（端末内）／レア度別演出／実績バッジ。撮影写真は結果カード表示＋トグルで図鑑にサムネ保存／ナンバープレートはVisionで領域推定→自動モザイク＋タップ手動追加（端末に残る写真は必ずぼかし版）。Vanilla JS+PWA、Vercel関数で鍵秘匿+レート制限(60秒10回)+サイズ上限。鍵ありで本物/なしでデモモック。現実世界図鑑シリーズの第1弾エンジン |
| TrainDex | `train-dex/` | デプロイ済み（Day28・3ゲート通過・本番疎通OK／https://traindex-kei.vercel.app）。RideDexエンジンを流用した鉄道図鑑。🚄新幹線/🚃在来線・私鉄/🚋路面電車の3カテゴリ。事業者+形式/系列+登場年+確度+ユーザー訂正/レア演出/実績(初/新幹線/路面電車/レア/10形式)。ナンバーぼかし→**写り込んだ人の顔の自動モザイク**に転用(粗め)+タップ手動追加。鍵あり=本物/なし=デモモック。現実世界図鑑シリーズ第2弾 |
| TransportDex | `transport-dex/` | デプロイ済み（Day29／https://transportdex-kei.vercel.app）。**車・鉄道・飛行機の統合プラットフォーム（顔は特化・中身は統合）**。1エンジン＋ジャンル設定`genres.js`で駆動＝ブランド/入力項目/カテゴリ/図鑑マスター/AIプロンプトを全部データ化。ハブ`/`(横断コレクション)＋ジャンル別`?g=train|ride|plane`。記録フォーム/図鑑/フライホイール/マスター図鑑/その他保持/レア演出は全ジャンル共通。API`/api/identify`はgenreでプロンプト切替（ジャンルallowlist）。**新ジャンルはgenres.jsに足すだけ**。改善が全ジャンルに複利で効く（統合の本来の動機）。マスター: train(新幹線16+在来24)/car(22)/plane(12) |
