...
| RideDex | `ride-dex/` | デプロイ済み（Day26・3ゲート通過・本番疎通OK／https://ridedex-kei.vercel.app）。街の車・バイクをカメラ→OpenAI Vision鑑定（メーカー/車種/世代+年式レンジ/確度%/レア度/豆知識）→図鑑コレクション。"宝探し"UX。年式は単年でなく世代レンジ／詳しい人が訂正可（端末内）／レア度別演出／実績バッジ。撮影写真は結果カード表示＋トグルで図鑑にサムネ保存／ナンバープレートはVisionで領域推定→自動モザイク＋タップ手動追加（端末に残る写真は必ずぼかし版）。Vanilla JS+PWA、Vercel関数で鍵秘匿+レート制限(60秒10回)+サイズ上限。鍵ありで本物/なしでデモモック。現実世界図鑑シリーズの第1弾エンジン |
