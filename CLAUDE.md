...
| 引き継ぎWiki職人 | `開発部/hikitsugi-wiki/` | 開発中 | Vanilla JS / PWA / localStorage(端末内のみ+控えJSON) / Vercel関数+OpenAI(gpt-4o-mini・鍵なしモック=コストゼロ)。退職・異動前の引き継ぎ書づくり: 資料持ち込み→AIが下書きWiki生成→**差分インタビュー**(書かれていないことだけ質問)→[[相互リンク]]Wiki→抜け漏れ検出+後任の30日プラン+HTML一式書き出し。送信前に機密らしき文字列を端末内で伏せ字化+同意モーダル。収益の芽=A型(期限駆動)。研究起点=Karpathy LLM Wiki |
