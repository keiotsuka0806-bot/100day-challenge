...
| 敷金ディフェンダー | `開発部/shikikin-defender/` | 開発中 | Vanilla JS / PWA / localStorage（写真は端末内のみ）。賃貸退去の原状回復チェック（国交省ガイドライン人手キュレーション・AI不使用＝コストゼロ）。負担区分＋経過年数の残存価値計算＋証拠アルバム＋交渉メモ印刷/コピー |

| 自由研究ラボ | `開発部/jiyukenkyu-lab/` | 共有可能 | Vanilla JS / PWA / localStorage(記録は端末内のみ) / Vercel関数+OpenAI(テーマ生成=gpt-4o・ヒント=gpt-4o-mini・鍵なしモック)。子どもの「好き×好きの掛け算」から自由研究テーマを作り、先行しらべ→問い→**だから何？ゲート**(比較相手+予想とのズレ=意味の検問)→はかり方→仮説→観察記録(対照実験ガード)→模造紙まとめまで30日伴走。研究はかせが全画面で伴走し**予想外れを最大の祝祭**にする。**AIは答え・考察を書かない＝親への安心が売り**。3ゲート通過(GC15/15→QA29件検出20件修正→セキュリティ92点)。設計の背骨=ISEF 2026最高賞の「4つの型」+Karpathy verifiability（https://jiyukenkyu-lab.vercel.app） |
