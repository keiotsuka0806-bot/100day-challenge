...
| 自由研究ラボ | `開発部/jiyukenkyu-lab/` | 開発中 | Vanilla JS / PWA / localStorage(記録は端末内のみ) / Vercel関数+OpenAI(gpt-4o-mini・鍵なしモック=コストゼロ)。子どもの「好き×好きの掛け算」から自由研究テーマを作り、問い→はかり方→仮説→観察記録→模造紙まとめまで30日伴走。設計の背骨=ISEF 2026最高賞のプロセスから抽出した「世界一の自由研究の4つの型」。**AIは答え・考察を書かない(問いを返すだけ)＝親への安心が売り**。研究起点=Karpathy verifiability |
